...
const updateContest = async (req, res) => {
    try {
        ...
        const contest = await Contest.findByIdAndUpdate(req.params, {
...
const addUser = async (req, res) => {
    ...
    try {
        ...
        if (
            contest.typeOfContest == 'private' &&
            contest.code == req.body.code
        ) {
            contest.users.push(req.Userdata._id);
            await contest.save();
            res.status(200).send({
                message: 'You are added to contest',
                success: true
            });
        } else if (contest) {
            contest.users.push(req.Userdata._id);
...
const removeUser = async (req, res) => {
    //removes to user from the contest
    try {
        const contest = await Contest.findById(req.params);
        if (contest.users.includes(Userdata._id)) {
            ...
            await contest.save();
            res.status(200).send({
                msg: 'you have been removed from contest',
                success: true
            });
        }
    } catch (e) {
